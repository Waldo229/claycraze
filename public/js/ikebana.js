...
async function loadIkebana() {
  try {
    const response = await fetch("/gallery-data/ikebana");
    ...
    if (!Array.isArray(pieces) || pieces.length === 0) {
      galleryGrid.innerHTML = `
        <div class="empty-state">
          No ikebana pieces are currently available for display.
        </div>
      `;
      return;
    }
    ...
    galleryGrid.innerHTML = `
      <div class="error-state">
        Could not load ikebana gallery data: ${escapeHtml(error.message)}
      </div>
    `;
    console.error("Ikebana gallery error:", error);
  }
}
