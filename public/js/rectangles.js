...
async function loadRectangles() {
  try {
    const response = await fetch("/gallery-data/rectangles");
    ...
    if (!Array.isArray(pieces) || pieces.length === 0) {
      galleryGrid.innerHTML = `
        <div class="empty-state">
          No rectangle pieces are currently available for display.
        </div>
      `;
      return;
    }
    ...
    galleryGrid.innerHTML = `
      <div class="error-state">
        Could not load rectangle gallery data: ${escapeHtml(error.message)}
      </div>
    `;
    console.error("Rectangle gallery error:", error);
  }
}
...
function buildCardHtml(piece) {
  ...
  const id = piece.id || "";
  const dimensions = formatRectDimensions(piece);
...
function formatRectDimensions(piece) {
